...
<section id=enroll-container>
		...
		<form id="memberFrm" method="post" >
			...
			<!-- /member/ModifyMember.do -->
			<input type="button" value="비밀번호수정" onclick="fn_updatePassword();" />
			<input type="button" value="정보수정" 
			onclick="fn_modifyMember();"/>
			<input type="button" value="탈퇴"/>
		</form>
	</section>
	
	<script type="text/javascript">
		const fn_modifyMember=()=>{
			/* memberFrm의 form태그의 action속성을 추가하여 submit하는 메소드 */
			$("#memberFrm").attr("action","<%=request.getContextPath()%>/member/ModifyMember.do")
			.submit();
		}
		
		const fn_updatePassword=()=>{
			//새창으로 패스워드 변경하기
			open("<%=request.getContextPath()%>/member/updatePassword.do?userId=<%=loginMember.getUserId()%>", "_blank"
